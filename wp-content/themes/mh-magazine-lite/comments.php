...
if (have_comments()) {
	...
	if (comments_open()) { ?>
		<h4 id="mh-comments" class="mh-widget-title mh-comment-form-title">
			<span class="mh-widget-title-inner">
				<?php esc_html_e('Будьте першим, поділіться враженням про публікацію.', 'mh-magazine-lite'); ?>
			</span>
		</h4><?php
	}
}
if (comments_open()) {
	comment_form(array(
		'title_reply' => esc_html__('Залишити відгук', 'mh-magazine-lite'),
        'comment_notes_before' => '<p class="comment-notes">' . esc_html__('Вашу email адресу не буде опубліковано.', 'mh-magazine-lite') . '</p>',
        'comment_notes_after'  => '',
        'comment_field' => '<p class="comment-form-comment"><label for="comment">' . esc_html__('Коментар', 'mh-magazine-lite') . '</label><br/><textarea id="comment" name="comment" cols="45" rows="5" aria-required="true"></textarea></p>'
	));
}
